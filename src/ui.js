...
class UI {
   ...
   * also, it validates that the page is opened using Rinkeby, which is the
  ...
  init = async () => {
    if (window.web3) {
      try {
        ...
        // Rinkeby ID
        if (netId !== 4) {
          this.containers.loading.innerHTML = rinkebyRequest;
          notify('This DApp needs to be loaded in Rinkeby');
          return;
        }
